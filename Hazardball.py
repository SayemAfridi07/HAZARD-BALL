...
def keyboardListener(key, x, y):
    ...
    global player_vel, game_over, player_pos, falling, is_first_person, score, lives, powerup_active, powerup_timer, level, diamonds_collected
    ...
    if key == b'c': 
        level += 1
        diamonds_collected = 0 # Reset diamonds for the new level
        ...
        # Generate the new level layout
        init_map() 
        print(f"*** CHEAT ACTIVATED: Skipped to Level {level} ***")
    ...
    if key == b'r': # Reset Game
        ...
        falling = False
        level = 1 # Reset level
        diamonds_collected = 0 # Reset diamonds
